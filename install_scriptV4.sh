...
if ! dpkg -s python3 moc screen >/dev/null 2>&1; then
    sudo apt-get install -y python3 moc screen
else
    echo "Python3, MOC, and Screen are already installed."
fi
sleep 5

# Install additional dependencies
echo "Installing additional dependencies..."
sudo apt-get install -y unzip libao-dev libdbus-1-dev moc-ffmpeg-plugin
sleep 5
...
if ! command -v vgmplay >/dev/null 2>&1; then
    ...
    echo "VGMPlay is already installed."
fi
sleep 5

# Add the Python scripts
echo "Adding Python scripts..."
sudo tee /usr/local/bin/vgmplay-pvgmp4f.py > /dev/null << 'EOF'
#!/usr/bin/env python3
...
def handle_vgz(folder_path):
    ...
    if vgz_files:
        try:
            command = ['screen', '-dmS', SCREEN_SESSION_NAME, 'vgmplay'] + vgz_files
...
sudo tee /usr/local/bin/mocp-pvgmp4f.py > /dev/null << 'EOF'
#!/usr/bin/env python3
...
sudo tee /usr/local/bin/music_player.py > /dev/null << 'EOF'
#!/usr/bin/env python3
...
sudo tee /etc/systemd/system/music_player.service > /dev/null << 'EOF'
[Unit]
...
sudo tee /etc/udev/rules.d/99-music-player.rules > /dev/null << 'EOF'
...
if ! grep -q "music ALL=NOPASSWD: /usr/bin/mount" /etc/sudoers; then
    echo "music ALL=NOPASSWD: /usr/bin/mount" | sudo tee -a /etc/sudoers > /dev/null
fi
if ! grep -q "music ALL=NOPASSWD: /usr/bin/umount" /etc/sudoers; then
    echo "music ALL=NOPASSWD: /usr/bin/umount" | sudo tee -a /etc/sudoers > /dev/null
fi
...
if [ ! -d /var/log/music_player ]; then
    sudo mkdir /var/log/music_player
fi
sudo chown music:music /var/log/music_player
sudo touch /var/log/music_player/music_player.log
sudo chown music:music /var/log/music_player/music_player.log
